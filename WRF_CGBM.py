...
if mode == 0:
    ...
    # plt.ion()
if mode == 1:
    modis_path             = 'H:/UHB_WRF_DATA/Yakou_data/'
    out_path_Temperature   = 'H:/UHB_WRF_DATA/Yakou_data/Temperature/'
    out_path_Windspeed     = 'H:/UHB_WRF_DATA/Yakou_data/Windspeed/'
    out_path_Precipitation = 'H:/UHB_WRF_DATA/Yakou_data/Precipitation/'

modisdata= Dataset(modis_path+'UHB_CGBM201401.nc', format='NETCDF4')
newcols  = 226
newrows  = 157
...
for i_date in dates:
    ...
    if temp_year != iyear or temp_month != imonth:        
        if i_date>=datetime.date(2014,1,1) and i_date<datetime.date(2015,1,1):
            modisdata.close()
            modisdata  = Dataset(modis_path+'UHB_CGBM'+str(iyear)+'%02d'%(imonth)+'.nc', format='NETCDF4')
            #read data
            PREC_dataset = modisdata.variables['forc_prec']#针对降水量数据需要累加
            T2_dataset   = modisdata.variables['forc_t']
            U10_dataset  = modisdata.variables['forc_us']
            V10_dataset  = modisdata.variables['forc_vs']
